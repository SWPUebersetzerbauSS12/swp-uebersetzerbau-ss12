...
public class RegexToDfaConverter {
	...
	public static SyntaxDirectedDefinition getRegexSdd() {
		...
		semanticRules.add( new SemanticRule() {	
			public void apply( AttributesMap... attributesMaps) {
				...
				TreeNode nodeS = (TreeNode) attributesMaps[2].get( "node");
	...
	public static <StatePayloadType extends Serializable> FiniteStateMachine<Character, StatePayloadType> convert( String regex, StatePayloadType payload)
		...
		try {
			...
			Grammar regexGrammar = Grammars.getRegexGrammar();
			
			AbstractSyntaxTree ast = new AbstractSyntaxTree( Grammars.getRegexGrammar(), getRegexSdd(), regex);
			System.out.println( ast.getRootAttributesMap().get( "node"));
